...
void AdServing::MaybeServeAd(const std::string& size,
  ...
  eligible_ads_->GetForSegments(
      ...
      [=](const bool was_allowed, const CreativeInlineContentAdList& ads) {
        ...
        // TODO(tmancey): This should only happen when the ad is viewed, so we
        // need to further discuss a new kServed type and then only add kViewed
        // once the ad has been viewed
